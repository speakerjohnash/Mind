...
def batch_to_tensor(config, batch):
	...
	thoughts = np.zeros(shape=(batch_size, 1, alphabet_length, doc_length))
	
	for index, t in enumerate(docs):
		thoughts[index][0] = string_to_tensor(config, t, doc_length)

	thoughts = np.transpose(thoughts, (0, 1, 3, 2))
...
def evaluate_testset(config, graph, sess, model, test):
	...
	for i in range(num_chunks):
		...
		thought_test, labels_test = batch_to_tensor(config, batch_test)
		feed_dict_test = {get_tensor(graph, "x:0"): thought_test}
...
def build_graph(config):
	...
	with graph.as_default():
		...
		thought_placeholder = tf.placeholder(tf.float32, shape=input_shape, name="x")
		...
		network_corr, details_corr = encoder(thought_placeholder, "network_corr", train=True, noise_std=0.075)

		logging.info("Clean Encoder")
		network_clean, details_clean = encoder(thought_placeholder, "network_clean", train=True)

		logging.info("Trained Model")
		trained_model, _ = encoder(thought_placeholder, "model", train=False)
...
def train_model(config, graph, sess, saver):
	...
	for step in range(num_eras):
		...
		unlabeled_thoughts, _ = batch_to_tensor(config, unlabeled_batch)
		all_thoughts = np.vstack([thoughts, unlabeled_thoughts])
		feed_dict = {
			get_tensor(graph, "x:0") : all_thoughts,
		...
		if step % 1000 == 0:
			predictions = sess.run(get_tensor(graph, "model:0"), feed_dict={get_tensor(graph, "x:0"): thoughts, get_tensor(graph, "y:0"): labels})
